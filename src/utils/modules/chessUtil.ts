 ...
 * @returns
 */
export  function getWinNum (winNumContext: ValidWinNumContext) : number {
    const { rowIndex, colIndex, board, winNum } = winNumContext;
    let count: number = 0;
    for (let index = 0; index < VECTORS.length; index++) {
        const enoughNumber = backDirection({ rowIndex, colIndex, board, vectorsIndex: VECTORS[index], winNum });
        if (enoughNumber) {
...
function getCountNum (getCountNumContext: ValidGetCountNumContext) : number {
    ...
    while (rowIndex < board.length && rowIndex >= 0 && colIndex < board[0].length && colIndex >= 0 && board[rowIndex][colIndex] === pieceType && pieceType) {
        count++;
